...
{
	uart2_send_byte(ch);			//将printf的底层重定向到自己的发送字节函数
	return ch;
}
...
{
	Uart2.RxBuffer[Uart2.RxDataCnt++] = Byte;   //接收数据转存
	
	if(Uart2.RxDataCnt > RX2_BUFFER_SIZE) 	//判断接收数据是否溢出
	{
		memset(Uart2.RxBuffer,0x00,sizeof(Uart2.RxBuffer));	//清除缓存
		Uart2.RxDataCnt = 0;	//下标置0，重新开始接收
	}				

	if((Uart2.RxBuffer[Uart2.RxDataCnt - 2] == '\r' && Uart2.RxBuffer[Uart2.RxDataCnt - 1] == '\n')) //判断结束位
	{
		uart2_send_string((char *)Uart2.RxBuffer);			//打印接收到的数据
		memset(Uart2.RxBuffer,0x00,sizeof(Uart2.RxBuffer)); //清空数组
		Uart2.RxDataCnt = 0;								
	}	
}
